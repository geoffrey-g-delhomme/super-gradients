...
from super_gradients.module_interfaces import IntersectEstimationPredictions, AbstractIntersectEstimationPostPredictionCallback
...
__all__ = ["IntersectEstimationMetrics"]


@register_metric(Metrics.INTERSECT_ESTIMATION_METRICS)
class IntersectEstimationMetrics(Metric):
    """
    ...
    >>> from super_gradients.training.metrics import IntersectEstimationMetrics
    >>> metric = IntersectEstimationMetrics(...)
    >>> metric.update(...)
    ...
    >>> from super_gradients.training.metrics import IntersectEstimationMetrics
    >>> metric = IntersectEstimationMetrics(iou_thresholds_to_report=[0.5, 0.75], ...)
    >>> metric.update(...)
    ...
    def __init__(
        self,
        post_prediction_callback: AbstractIntersectEstimationPostPredictionCallback,
        num_joints: int,
    ...
    def update(
        ...
        """
        predictions: List[IntersectEstimationPredictions] = self.post_prediction_callback(preds)  # Decode raw predictions into poses
    ...
    def _update_with_samples(self, predictions: List[IntersectEstimationPredictions], gt_samples: List[PoseEstimationSample]) -> None:
        """
        ...
        for i in range(len(predictions)):
            self.update_single_image(
                ...
                predicted_lines=predictions[i].lines,
    ...
    def _update_with_old_style_args(
        self,
        predictions: List[IntersectEstimationPredictions],
    ...
    def update_single_image(
        ...
        predicted_lines: Union[Tensor, np.ndarray],
        ...
        :param predicted_lines:  Predicted poses of shape (num_instances, num_lines, 2, 3)
        ...
        predicted_lines = convert_to_tensor(predicted_scores, dtype=torch.float32, device="cpu")
