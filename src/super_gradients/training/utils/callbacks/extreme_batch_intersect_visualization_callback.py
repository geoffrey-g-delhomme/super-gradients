...
if typing.TYPE_CHECKING:
    ...
    from super_gradients.module_interfaces import IntersectEstimationPredictions


@register_callback("ExtremeBatchIntersectEstimationVisualizationCallback")
class ExtremeBatchIntersectEstimationVisualizationCallback(ExtremeBatchCaseVisualizationCallback):
    """
    ...
    It requires a key 'gt_samples' (List[IntersectEstimationSample]) to be present in additional_batch_items dictionary.

    Supported models: YoloNASIntersect
    Supported datasets: COCOIntersectEstimationDataset
    ...
    def __init__(
        ...
        line_colors: Union[ListConfig, List[Tuple[int, int, int]]],
        ...
        self.line_colors = list(tuple(map(int, color)) for color in line_colors)
    ...
    def _visualize_batch(
        ...
        lines: List[Union[np.ndarray, Tensor]],
        ...
        for i in range(image_tensor.shape[0]):
            ...
            if torch.is_tensor(lines_i):
                lines_i = lines_i.detach().cpu().numpy()
            ...
            res_image = IntersectVisualization.draw_poses(
                image=res_image,
                poses=keypoints_i,
                boxes=bboxes_i,
                scores=scores_i,
    ...
    def process_extreme_batch(self) -> np.ndarray:
        ...
        predictions: List[IntersectEstimationPredictions] = self.post_prediction_callback(self.extreme_preds)

        images_to_save_preds = self._visualize_batch(
            image_tensor=inputs,
            keypoints=[p.poses for p in predictions],
            lines=[p.lines for p in predictions],
        ...
        for gt in gt_samples:
            ...
            for kpts in gt.joints:
                ...
                for edge, o in zip((kpts[[0, 1]], kpts[[7, 6]], kpts[[5, 4]]), LINE_BORDER_ORIENTATIONS):
                    line = [0] * 5
