...
from super_gradients.training.utils.intersect_utils import LINE_BORDER_ORIENTATIONS, border2image, image2border
...
# DEBUG
ENABLE_LINE_OKS_IN_TASK_ASSIGNER = False
LINE_OKS_SIGMA = 0.1


@dataclasses.dataclass
class YoloNASIntersectYoloNASIntersectBoxesAssignmentResult:
    """
    This dataclass stores result of assignment of predicted boxes to ground truth boxes for YoloNASIntersect model.
    It produced by YoloNASIntersectTaskAlignedAssigner and is used by YoloNASIntersectLoss to compute the loss.
...
class YoloNASIntersectTaskAlignedAssigner(nn.Module):
    """
    ...
    def forward(
        ...
        _, _, num_lines, _ = pred_line_coords.shape
        ...
        if num_max_boxes == 0:
            ...
            return YoloNASIntersectYoloNASIntersectBoxesAssignmentResult(
...
@register_loss(Losses.YOLONAS_INTERSECT_LOSS)
class YoloNASIntersectLoss(nn.Module):
    """
    Loss for training YoloNASIntersect model
    """
    ...
    def forward(
        ...
        loss_iou, loss_dfl, loss_pose_cls, loss_pose_reg, loss_line_cls, loss_line_reg = self._bbox_loss(
            pred_distri,
            pred_bboxes,
            pred_pose_coords=pred_pose_coords,
            pred_pose_logits=pred_pose_logits,
            pred_line_coords=pred_line_coords,
            pred_line_logits=pred_line_logits,
            stride_tensor=stride_tensor,
    ...
    def component_names(self):
        return ["loss_cls", "loss_iou", "loss_dfl", "loss_pose_cls", "loss_pose_reg", "loss_line_cls", "loss_line_reg", "loss"]
    ...
    def _get_kpt_projected_on_line(self, line_from: Tensor, border_from: str, line_to: Tensor, border_to: str, kpt: Tensor, anchor_points: Tensor, stride_tensor: Tensor) -> Tensor:
        ...
        Args:
            line_from (Tensor): _description_
            border_from (str): _description_
            line_to (Tensor): _description_
            border_to (str): _description_
            kpt (Tensor): _description_
    ...
    def _intersect_loss(
        ...
        target_pose_coords: Tensor,
        predicted_line_coords: Tensor,
        target_line_coords: Tensor,
        predicted_line_logits: Tensor,
        target_line_visibility: Tensor,
        ...
        if self.line_regression_loss_type == "bce":
            ...
            regression_loss_unreduced_list = []
            for i in range(predicted_line_coords.shape[-1]):
                d = ((predicted_line_coords[..., i] - target_line_coords[..., i]) ** 2).sum(dim=-1, keepdim=True)  # [[Num Instances, Num Joints, 1]
                # e = d / (2 * sigmas) ** 2 / (area + 1e-9) / 2  # [Num Instances, Num Joints, 1]
                e = d / (2 * sigmas) ** 2 / perim / 2  # [Num Instances, Num Joints, 1]
                regression_loss_unreduced_item = 1 - torch.exp(-e)  # [Num Instances, Num Joints, 1]
                regression_loss_unreduced_list.append(regression_loss_unreduced_item)
        ...
        if self.line_regression_loss_clip_weight:
            ...
            for i_line in range(predicted_line_coords.shape[-2]):
                ...
                for kpt_indice in kpt_indices_list[i_line]:
                    ...
                    elif "mse" in self.line_regression_loss_type:
                        regression_loss_unreduced = torch.nn.functional.mse_loss(kpt, proj, reduction="none").sum(dim=-1, keepdim=True) / perim
                    visible_mask = line_visible_targets_mask[:, i_line, :] * pose_visible_targets_mask[..., kpt_indice, :]
                    regression_loss_reduced = (regression_loss_unreduced * visible_mask).sum(dim=-1, keepdim=False) / (
                        visible_mask.sum(dim=1, keepdim=False) + 1e-9
                    )
    ...
    def _bbox_loss(
        ...
        stride_tensor,
        anchor_points,
        assign_result: YoloNASIntersectYoloNASIntersectBoxesAssignmentResult,
        assigned_scores_sum,
        reg_max: int,
    ):
        ...
        if num_pos > 0:
            ...
            if self.line_regression_loss_type == "proj_oks":
                ...
                for kpts, bounds, diag in zip(masked_gt_poses.cpu().numpy(), masked_bounds.cpu().numpy(), masked_diag.cpu().numpy()):
                    ...
                    for i_line, (edge, o) in enumerate(zip((kpts[[0, 1]], kpts[[7, 6]], kpts[[5, 4]]), LINE_BORDER_ORIENTATIONS)):
                        line = [[0, 0], [0, 0]]
            ...
            loss_line_cls = torch.zeros([], device=pred_bboxes.device)
            loss_line_reg = torch.zeros([], device=pred_bboxes.device)

        return loss_iou, loss_dfl, loss_pose_cls, loss_pose_reg, loss_line_cls, loss_line_reg
