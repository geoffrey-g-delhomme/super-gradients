...
YoloNasIntersectDecodedPredictions = Tuple[Tensor, Tensor, Tensor, Tensor]
YoloNasIntersectRawOutputs = Tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, List[int], Tensor]


@register_detection_module()
class YoloNASIntersectNDFLHeads(BaseDetectionModule, SupportsReplaceNumClasses):
    def __init__(
        self,
        num_classes: int,
    ...
    def forward(self, feats: Tuple[Tensor, ...]) -> Union[YoloNasIntersectDecodedPredictions, Tuple[YoloNasIntersectDecodedPredictions, YoloNasIntersectRawOutputs]]:
        """
        ...
        - pred_line_scores [B, Num Anchors, Num Lines, 2] - Predicted from / to intersection points per line
        - pred_line_scores [B, Num Anchors, Num Line] - Predicted scores for each line
        ...
        pose_logits_list = []
        line_regression_list = []
        line_logits_list = []

        for i, feat in enumerate(feats):
            b, _, h, w = feat.shape # h, w: number of cells (multiply by stride for image resolution)
            height_mul_width = h * w
            reg_distri, cls_logit, pose_regression, pose_logits, line_regression, line_logits = getattr(self, f"head{i + 1}")(feat)
            ...
            line_regression_list.append(torch.permute(line_regression.flatten(3), [0, 3, 1, 2]))  # [B, L, 2, H, W] -> [B, H * W, L, 2]
            line_logits_list.append(torch.permute(line_logits.flatten(2), [0, 2, 1]))  # [B, L, H, W] -> [B, H * W, L]
        ...
        line_regression_list = torch.cat(line_regression_list, dim=1)  # [B, Anchors, L, 2]
        line_logits_list = torch.cat(line_logits_list, dim=1)  # [B, Anchors, L]
