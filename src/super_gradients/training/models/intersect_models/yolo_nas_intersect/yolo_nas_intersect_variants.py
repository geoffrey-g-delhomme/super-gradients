...
from super_gradients.module_interfaces.exportable_intersect_estimation import AbstractIntersectEstimationDecodingModule, ExportableIntersectEstimationModel
...
from super_gradients.training.pipelines.pipelines import IntersectEstimationPipeline
...
class YoloNASIntersectDecodingModule(AbstractIntersectEstimationDecodingModule):
    ...
    def infer_total_number_of_predictions(self, inputs: Any) -> int:
        ...
        :param inputs: YoloNASIntersect model outputs
        :return:
        """
        if torch.jit.is_tracing():
            pred_bboxes_xyxy, pred_bboxes_conf, pred_pose_coords, pred_pose_scores, pred_line_coords, pred_line_scores = inputs
        else:
            pred_bboxes_xyxy, pred_bboxes_conf, pred_pose_coords, pred_pose_scores, pred_line_coords, pred_line_scores = inputs[0]
    ...
    def forward(self, inputs: Tuple[Tuple[Tensor, Tensor], Tuple[Tensor, ...]]):
        """
        Decode YoloNASIntersect model outputs into bounding boxes, confidence scores and pose coordinates and scores
        ...
        pred_lines_and_scores = torch.cat([pred_line_coords, pred_line_scores.unsqueeze(3)], dim=3)
        ...
        output_pred_lines = pred_lines_and_scores.reshape(-1, pred_lines_and_scores.size(2), 3)[flat_indices, :, :].reshape(
            pred_lines_and_scores.size(0), nms_top_k, pred_lines_and_scores.size(2), pred_lines_and_scores.size(3)
        )

        return output_pred_bboxes, output_pred_scores, output_pred_joints, output_pred_lines


class YoloNASIntersect(CustomizableDetector, ExportableIntersectEstimationModel, SupportsInputShapeCheck):
    """
    YoloNASIntersect model
    ...
    def get_decoding_module(self, num_pre_nms_predictions: int, **kwargs) -> AbstractIntersectEstimationDecodingModule:
        return YoloNASIntersectDecodingModule(num_pre_nms_predictions)
    ...
    def _get_pipeline(
        ...
        pipeline = IntersectEstimationPipeline(
            model=self,
    ...
    def get_post_prediction_callback(
        ...
        return YoloNASIntersectPostPredictionCallback(
...
@register_model(Models.YOLO_NAS_INTERSECT_N)
class YoloNASIntersect_N(YoloNASIntersect):
    def __init__(self, arch_params: Union[HpmStruct, DictConfig]):
        default_arch_params = get_arch_params("yolo_nas_intersect_n_arch_params")
...
@register_model(Models.YOLO_NAS_INTERSECT_S)
class YoloNASIntersect_S(YoloNASIntersect):
    def __init__(self, arch_params: Union[HpmStruct, DictConfig]):
        default_arch_params = get_arch_params("yolo_nas_intersect_s_arch_params")
        merged_arch_params = HpmStruct(**copy.deepcopy(default_arch_params))
        merged_arch_params.override(**arch_params.to_dict())
        super().__init__(
            backbone=merged_arch_params.backbone,
...
@register_model(Models.YOLO_NAS_INTERSECT_M)
class YoloNASIntersect_M(YoloNASIntersect):
    def __init__(self, arch_params: Union[HpmStruct, DictConfig]):
        default_arch_params = get_arch_params("yolo_nas_intersect_m_arch_params")
        merged_arch_params = HpmStruct(**copy.deepcopy(default_arch_params))
        merged_arch_params.override(**arch_params.to_dict())
        super().__init__(
            backbone=merged_arch_params.backbone,
...
@register_model(Models.YOLO_NAS_INTERSECT_L)
class YoloNASIntersect_L(YoloNASIntersect):
    def __init__(self, arch_params: Union[HpmStruct, DictConfig]):
        default_arch_params = get_arch_params("yolo_nas_intersect_l_arch_params")
        merged_arch_params = HpmStruct(**copy.deepcopy(default_arch_params))
        merged_arch_params.override(**arch_params.to_dict())
        super().__init__(
            backbone=merged_arch_params.backbone,
