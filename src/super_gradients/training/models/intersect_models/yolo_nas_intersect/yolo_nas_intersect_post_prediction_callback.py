...
from super_gradients.module_interfaces import AbstractIntersectEstimationPostPredictionCallback, IntersectEstimationPredictions


class YoloNASIntersectPostPredictionCallback(AbstractIntersectEstimationPostPredictionCallback):
    """
    ...
    @torch.no_grad()
    def __call__(self, outputs: Tuple[Tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor], ...]) -> List[IntersectEstimationPredictions]:
        """
        ...
        decoded_predictions: List[IntersectEstimationPredictions] = []
        for pred_bboxes_xyxy, pred_bboxes_conf, pred_pose_coords, pred_pose_scores, pred_line_coords, pred_line_scores in zip(*predictions):
            ...
            pred_line_coords = pred_line_coords[conf_mask].float()
            pred_line_scores = pred_line_scores[conf_mask].float()
            ...
            if pred_bboxes_conf.size(0) > self.pre_nms_max_predictions:
                ...
                pred_line_coords = pred_line_coords[topk_candidates.indices]
                pred_line_scores = pred_line_scores[topk_candidates.indices]
            ...
            final_lines = torch.cat(
                ...
                dim=-1,
            )  # [Instances, Num Keypoints, 5] (x_from, y_from, x_to, y_to, conf)
            ...
            decoded_predictions.append(
                IntersectEstimationPredictions(
                    ...
                    lines=final_lines[: self.post_nms_max_predictions],
